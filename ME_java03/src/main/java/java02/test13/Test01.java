/* Inner class
   ...
   => 특정 클래스 안에서만 사용하는 클래스인 경우
      그 클래스 내부에서 정의함으로써 외부에 노출되는 것을 막을 수 있다.
      => 관리가 쉽다!
...
public class Test01 {
	...
	static void x() {
		i = 100;  // ok
		//j = 100;  // error => static 메서드나 블록은 this라는 내장 변수가 없어서..
	}
	
	void y() {
		i = 100;  // ok => 인스턴스 메서드나 블록은 클래스 변수에 바로 접근할 수 있다.
		j = 100;  // ok => 인스턴스 메서드나 블록은 this라는 내장 변수가 있다! (다만 생략되었음)
	}
	...
	class MemberInnerClass {
		...
		public void test() {
			// 당연히 non-static 블록에서는 static 멤버 사용 가능.
			i = 200;
			
			// member 이너 클래스에서는 바깥 클래스의 인스턴스 멤버를 사용할 수 있다.
			j = 300;
