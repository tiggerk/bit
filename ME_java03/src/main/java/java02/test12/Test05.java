/* 크리티컬 섹션(critical section)을 동기화 처리하기
   => 여러 스레드가 크리티컬 섹션 부분을 실행하더라도 문제없게 만들기
   => 한 번에 하나의 스레드만 접근하게 만듦.
 ...
 * 여러 스레드가 진입하더라도 계산에 영향을 끼치지 않는 코드 블록
   ...
   => Thread safe (스레드에 안전하다) (하다면 synchronized가 필요없다)
   => 동기화 처리를 할 필요가 없다.
...
public class Test05 {
	static class Account {
		...
		private long delay() {  // 아무런 의미 없는 시간 죽이기 코드임!
			long l = 10L;
			double b = 3.15;
			b = b / l;
