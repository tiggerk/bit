...
/* 서블릿 생성
 * => 클라이언트(웹브라우저)가 요청했을 때 생성된다. 요청하지 않으면 생성 안된다.
 * 
 * 클라이언트 요청이 없더라도 강제로 서블릿을 생성하는 방법
 * => 웹 애플리케이션을 시작할 때 생성된다.
 * => 배치(web.xml) 설정에 <load-on-startup>을 추가한다.
...
@WebServlet(
    name="AppInitServlet",
    loadOnStartup=1  // 소스를 열고 변경하고 저장하고 컴파일해야하는 단점이 있음.
)
*/
